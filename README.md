...
### SkillPay.me Integration (Optional but Recommended)
...
- Enable automated revenue collection

**Billing model** (SkillPay):
- **Rate**: 1 USDT = 1000 API calls (across all features)
- **Feature multipliers**:
  - `fetchTweet`: 1 unit per call
  - `fetchThread`: 3 units per call (counts as 3 calls)
  - `fetchTimeline`: 1 unit per 50 tweets
  - `monitorUser`: 1 unit per check
- **Example**: 1000 `fetchTweet` calls = 1000 units = 1 USDT

Your pricing on ClawHub:
- Pro plan: $1.9/month for 1000 calls → 1.9 USDT revenue per subscriber
- Business plan: $9.9/month unlimited → 9.9 USDT revenue per subscriber

**Without SkillPay**: Users upgrade manually (you manage subscriptions yourself).  
**With SkillPay**: Fully automated, users pay via crypto (BNB Chain USDT), you receive USDT with 5% platform fee.
